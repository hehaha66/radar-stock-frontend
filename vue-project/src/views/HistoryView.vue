<template>
  <div class="history-container" :class="{ 'panel-collapsed': isPanelCollapsed }">
    <!-- Left Control Panel -->
    <div class="control-panel">
      <!-- Collapse Button -->
      <el-button
        :icon="isPanelCollapsed ? DArrowRight : DArrowLeft"
        class="collapse-btn"
        circle
        @click="togglePanel"
      />

      <!-- Real-time Info Display Panel (Visible when collapsed) -->
      <div v-if="isPanelCollapsed" class="info-display-panel">
        <el-card class="box-card" shadow="never">
          ...
          <!-- Use `displayData` with optional chaining (`?.`) for safety -->
          <div v-if="displayData" class="info-content">
            <div class="info-date">{{ displayData?.date }}</div>
            <div class="info-group">
              <div class="info-row">
                <span>开: <span :class="getOhlcColorClass(displayData)">{{ displayData?.open }}</span></span>
                <span>收: <span :class="getOhlcColorClass(displayData)">{{ displayData?.close }}</span></span>
              </div>
              <div class="info-row">
                <span>低: <span :class="getOhlcColorClass(displayData)">{{ displayData?.low }}</span></span>
                <span>高: <span :class="getOhlcColorClass(displayData)">{{ displayData?.high }}</span></span>
              </div>
            </div>
            <el-divider />
            <div class="info-group">
              <div v-if="displayData?.ma5" class="info-line"><span>MA5:</span> <span class="value-ma5">{{ displayData?.ma5 }}</span></div>
              <div v-if="displayData?.ma10" class="info-line"><span>MA10:</span> <span class="value-ma10">{{ displayData?.ma10 }}</span></div>
              <div v-if="displayData?.ma20" class="info-line"><span>MA20:</span> <span class="value-ma20">{{ displayData?.ma20 }}</span></div>
            </div>
            <el-divider v-if="displayData?.volume" />
            <div v-if="displayData?.volume" class="info-group">
              <div class="info-line"><span>成交量:</span> <span class="value-vol">{{ displayData?.volume }}</span></div>
            </div>
            <el-divider v-if="displayData?.dif" />
            <div v-if="displayData?.dif" class="info-group">
                <div class="info-line"><span>DIF:</span> <span class="value-dif">{{ displayData?.dif }}</span></div>
                <div class="info-line"><span>DEA:</span> <span class="value-dea">{{ displayData?.dea }}</span></div>
                <div class="info-line"><span>MACD Hist:</span> <span :class="displayData?.hist > 0 ? 'value-up' : 'value-down'">{{ displayData?.hist }}</span></div>
            </div>
            <el-divider v-if="displayData?.k" />
            <div v-if="displayData?.k" class="info-group">
              <div class="info-line"><span>K:</span><span class="value-kdj-k">{{ displayData?.k }}</span></div>
              <div class="info-line"><span>D:</span><span class="value-kdj-d">{{ displayData?.d }}</span></div>
              <div class="info-line"><span>J:</span><span class="value-kdj-j">{{ displayData?.j }}</span></div>
            </div>
            <el-divider v-if="displayData?.rsi" />
            <div v-if="displayData?.rsi" class="info-group">
              <div class="info-line"><span>RSI:</span><span class="value-rsi">{{ displayData?.rsi }}</span></div>
            </div>
            <el-divider v-if="displayData?.wr" />
            <div v-if="displayData?.wr" class="info-group">
              <div class="info-line"><span>WR:</span><span class="value-wr">{{ displayData?.wr }}</span></div>
            </div>
          </div>
          <el-empty v-else description="请选择股票或等待数据加载" :image-size="50" />
        </el-card>
      </div>

      <!-- Original Panels (Visible when not collapsed) -->
      <div v-show="!isPanelCollapsed" class="original-panels">
        <!-- Favorite Stocks -->
        <el-card class="box-card" shadow="never">
          <template #header>
            <div class="card-header">
          ...
          >
            <!-- [MODIFIED] Simplified display to show only the code -->
            <span class="stock-item-code-only">{{ stock.code }}</span>
                <el-button
                  type="danger"
                  :icon="Delete"
                  circle
        ...
        <el-card class="box-card" shadow="never" v-if="selectedStock">
          ...
          <el-form label-position="top">
            ...
            <el-form-item label="数据周期:">
              <el-radio-group v-model="dataPeriod" @change="loadHistoryData" style="width: 100%" size="small">
            ...
            <el-form-item label="复权方式:">
              <el-radio-group v-model="adjustType" @change="loadHistoryData" style="width: 100%" size="small">
        ...
        <el-card class="box-card" shadow="never" v-if="selectedStock">
          ...
          <el-checkbox-group v-model="selectedIndicators" @change="updateChart" class="indicator-checkbox-group">
              <el-checkbox label="volume" size="small">成交量</el-checkbox>
              <el-checkbox label="macd" size="small">MACD</el-checkbox>
              <el-checkbox label="rsi" size="small">RSI</el-checkbox>
              <el-checkbox label="kdj" size="small">KDJ </el-checkbox>
              <el-checkbox label="wr" size="small">WR</el-checkbox>
            </el-checkbox-group>
        </el-card>
      </div>
    </div>
...
// A computed property to intelligently display data
const displayData = computed(() => {
  return hoveredData.value || latestData.value
})
...
function togglePanel() {
  ...
  // Use a timeout to ensure the resize happens after the CSS transition
  setTimeout(() => {
    mainChart?.resize();
...
function getOhlcColorClass(data: any) {
  if (!data || data.open === undefined || data.close === undefined) {
    return '';
  }
  return parseFloat(data.close) >= parseFloat(data.open) ? 'value-up' : 'value-down';
}
...
async function loadHistoryData() {
  ...
  try {
    ...
    const response: any = await api.get('/data/download-history', { params });
...
function renderCharts() {
  ...
  const { dates, klineData, volumes, ma5, ma10, ma20, macd, rsi, kdj, wr, rawData } = processedData.value;

  const lastIndex = rawData.length - 1;
  if (lastIndex >= 0) {
      const lastRawItem = rawData[lastIndex];
      latestData.value = {
        date: lastRawItem.date,
        open: lastRawItem.open.toFixed(2),
        close: lastRawItem.close.toFixed(2),
        low: lastRawItem.low.toFixed(2),
        high: lastRawItem.high.toFixed(2),
        volume: lastRawItem.volume.toLocaleString(),
        ma5: ma5[lastIndex]?.toFixed(2),
        ma10: ma10[lastIndex]?.toFixed(2),
        ma20: ma20[lastIndex]?.toFixed(2),
        dif: macd.macd[lastIndex]?.toFixed(2),
        dea: macd.signal[lastIndex]?.toFixed(2),
        hist: macd.histogram[lastIndex]?.toFixed(2),
        k: kdj.k[lastIndex]?.toFixed(2),
        d: kdj.d[lastIndex]?.toFixed(2),
        j: kdj.j[lastIndex]?.toFixed(2),
        rsi: rsi[lastIndex]?.toFixed(2),
        wr: wr[lastIndex]?.toFixed(2),
      };
  }
  ...
  const addIndicator = (indicatorName: string) => {
    ...
    switch(indicatorName) {
      case 'volume':
        series.push({ name: '成交量', type: 'bar', xAxisIndex: gridIndexCounter, yAxisIndex: gridIndexCounter, data: volumes, itemStyle: { color: (params: any) => (params.data[2] === 1 ? 'rgba(239, 68, 68, 0.7)' : 'rgba(16, 185, 129, 0.7)')}});
        titleText = '成交量';
        break;
      case 'macd':
        series.push(
  ...
  const option: EChartsOption = {
      ...
      tooltip: {
        ...
        formatter: (params: any): string => {
          ...
          if (processedData.value?.rawData?.[dataIndex] === undefined) {
            return '';
          }

          const rawDataItem = processedData.value.rawData[dataIndex];

          hoveredData.value = {
            date: rawDataItem.date,
            open: rawDataItem.open.toFixed(2),
            close: rawDataItem.close.toFixed(2),
            low: rawDataItem.low.toFixed(2),
            high: rawDataItem.high.toFixed(2),
            volume: rawDataItem.volume.toLocaleString(),
            ma5: processedData.value.ma5[dataIndex]?.toFixed(2),
            ma10: processedData.value.ma10[dataIndex]?.toFixed(2),
            ma20: processedData.value.ma20[dataIndex]?.toFixed(2),
            dif: processedData.value.macd.macd[dataIndex]?.toFixed(2),
            dea: processedData.value.macd.signal[dataIndex]?.toFixed(2),
            hist: processedData.value.macd.histogram[dataIndex]?.toFixed(2),
            k: processedData.value.kdj.k[dataIndex]?.toFixed(2),
            d: processedData.value.kdj.d[dataIndex]?.toFixed(2),
            j: processedData.value.kdj.j[dataIndex]?.toFixed(2),
            rsi: processedData.value.rsi[dataIndex]?.toFixed(2),
            wr: processedData.value.wr[dataIndex]?.toFixed(2),
          };
          ...
          params.forEach((item: any) => {
            if (item.seriesName && item.seriesName !== 'K线') {
               ...
               if (value !== undefined && value !== null) {
                 tooltipHtml += `${item.marker} ${item.seriesName}: <span style="font-weight: bold;">${value.toFixed(2)}</span><br/>`;
               }
            }
          });
...
.value-up { color: #ef4444; font-weight: 500; }
.value-down { color: #10b981; font-weight: 500; }
.value-ma5 { color: #38bdf8; font-weight: 500; }
.value-ma10 { color: #f59e0b; font-weight: 500; }
.value-ma20 { color: #8b5cf6; font-weight: 500; }
.value-vol { color: #e2e8f0; font-weight: 500; }
.value-dif { color: #38bdf8; font-weight: 500; }
.value-dea { color: #ef4444; font-weight: 500; }
.value-kdj-k { color: #38bdf8; font-weight: 500; }
.value-kdj-d { color: #ef4444; font-weight: 500; }
.value-kdj-j { color: #f59e0b; font-weight: 500; }
.value-rsi { color: #f59e0b; font-weight: 500; }
.value-wr { color: #a855f7; font-weight: 500; }
...
.stock-item.active .stock-item-code-only {
  color: #020617;
}

.stock-item-code-only {
  font-size: 0.9rem;
  font-weight: 500;
  color: #e2e8f0;
}
